...
use deep_faceswap_core::video::{
    ...
    save_face_records, scan_frames_for_faces_with_progress,
};
...
/// Video analyze job creation response (same shape as swap)
#[derive(Serialize, ToSchema)]
pub struct VideoAnalyzeJobResponse {
    #[schema(example = "job-abc123")]
    pub job_id: String,
    #[schema(example = "queued")]
    pub status: String,
...
/// Analyze video: extract frames, scan faces, cluster (async job)
#[utoipa::path(
    post,
    tag = "Video",
    ...
    responses(
        (status = 202, description = "Analyze job created", body = VideoAnalyzeJobResponse),
...
pub async fn analyze_video(
    ...
    if req.source_paths.is_empty() {
        return HttpResponse::BadRequest().json(ErrorResponse {
            error_text: "No source paths provided".to_string(),
        });
    }
    if req.target_video_path.is_empty() {
        return HttpResponse::BadRequest().json(ErrorResponse {
            error_text: "No target video path provided".to_string(),
        });
    }
    ...
    tokio::task::spawn_blocking(move || {
        // Update status to running
        {
            ...
            if let Some(job) = jobs.get_mut(&job_id_clone) {
                ...
                job.progress.stage = "detecting_source_faces".to_string();
            }
        }

        let result: Result<VideoAnalyzeResponse, String> = (|| {
            ...
            {
                let mut dirs = state_clone.session_dirs.lock().unwrap();
            ...
            // Update progress: extracting frames
            {
                let mut jobs = state_clone.jobs.lock().unwrap();
                if let Some(job) = jobs.get_mut(&job_id_clone) {
                    job.progress.stage = "extracting_frames".to_string();
                }
            }
            ...
            // Update progress: scanning faces
            {
                let mut jobs = state_clone.jobs.lock().unwrap();
                if let Some(job) = jobs.get_mut(&job_id_clone) {
                    job.progress.stage = "scanning_faces".to_string();
                    job.progress.total = total_frames;
                }
            }

            // Scan all frames for faces — with progress callback
            let state_for_scan = state_clone.clone();
            let job_id_for_scan = job_id_clone.clone();
            let scan_progress = move |current: usize, total: usize| {
                let mut jobs = state_for_scan.jobs.lock().unwrap();
                if let Some(job) = jobs.get_mut(&job_id_for_scan) {
                    job.progress.current = current;
                    job.progress.total = total;
                }
            };
            let mut face_records = scan_frames_for_faces_with_progress(
                &frame_paths,
                &mut detector,
                &mut recognizer,
                Some(&scan_progress),
            ...
            // Update progress: clustering
            {
                let mut jobs = state_clone.jobs.lock().unwrap();
                if let Some(job) = jobs.get_mut(&job_id_clone) {
                    job.progress.stage = "clustering".to_string();
                    job.progress.current = total_frames;
                }
            }
    ...
    HttpResponse::Accepted().json(VideoAnalyzeJobResponse {
        job_id,
